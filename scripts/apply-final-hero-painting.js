...
(function() {
  ...
  function ensureHeroBg() {
    if (!document.getElementById('kawaki-hero-background-layer') && document.body) {
      ...
      vignette.style.cssText = 'position: absolute; inset: 0; background: radial-gradient(circle at center, transparent 50%, rgba(0,0,0,0.45) 100%), linear-gradient(to bottom, rgba(0,0,0,0.2) 0%, transparent 15%, transparent 80%, rgba(0,0,0,0.7) 100%); pointer-events: none; transition: opacity 0.7s ease;';
  ...
  function ensureStyles() {
    if (!document.getElementById('kawaki-runtime-styles') && document.head) {
      ...
      style.textContent = \`
        /* Three.js canvas layer positioning and blending for hero opening */
        .sticky.top-0.left-0.w-full.h-screen {
          z-index: 1 !important;
          mix-blend-mode: screen;
        }

        body.scrolled-to-sidekick .sticky.top-0.left-0.w-full.h-screen {
          mix-blend-mode: normal;
        }

        /* Hide duplicate SVG title in center box so top title is crisp */
...
if (bodyEndIdx !== -1) {
  html = html.substring(0, bodyEndIdx) + finalHeroScript + html.substring(bodyEndIdx);
} else {
  html += finalHeroScript;
}

fs.writeFileSync('public/index.html', html, 'utf8');
console.log('Successfully written finalized hero painting script to public/index.html');
