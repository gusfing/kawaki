...
const server = http.createServer((req, res) => {
  ...
  if (reqPath === '/' || reqPath === '/editions' || reqPath === '/editions/winter2026' || reqPath === '/editions/winter2026/' || reqPath === '/editions/winter2026a55d') {
    const winterPage = path.join(ROOT, 'www.shopify.com', 'editions', 'winter2026a55d.html');
    if (fs.existsSync(winterPage)) {
      serveLocalFile(winterPage, res);
      return;
    }
  }
  ...
  fs.stat(filePath, (err, stats) => {
    ...
    // Check if path exists with .html extension
...
server.listen(PORT, () => {
  console.log(`Static server + fallback proxy running at http://localhost:${PORT}`);
});
